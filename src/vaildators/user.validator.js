...
export class UserValidator {
    static name = joi.string().regex(RegexpEnum.NAME).trim().messages({
    ...
    static surname = joi.string().regex(RegexpEnum.SURNAME).trim().messages({
    ...
    static age = joi.number().min(1).max(100).messages({
    ...
    static email = joi.string().regex(RegexpEnum.EMAIL).email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }).messages({
    ...
    static password = joi.string().regex(RegexpEnum.PASSWORD).messages({
    ...
    static gender = joi.string().valid(...Object.values(GenderEnum)).trim().messages({
        "string.empty": "gender must not be empty",
        "string.base": "gender must be string type",
        "any.only": "gender must be either male or female"
    });
    static phone = joi.string().regex(RegexpEnum.PHONE).trim().messages({
    ...
    static city = joi.string().regex(RegexpEnum.CITY).trim().messages({
    ...
    static region = joi.string().regex(RegexpEnum.REGION).trim().messages({
